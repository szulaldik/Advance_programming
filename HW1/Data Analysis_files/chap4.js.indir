...
function analyze(min=0) {
  ...
  for (let evt of EVENTS) {
    ...
    if (Math.abs(cor) > min)
      a.push(evt +": "+cor.toFixed(4)+ " " +JOURNAL.filter(x => x.events.includes(evt)).length)
  }
  return a
}

function seeSquirrelEvt(){
  
  JOURNAL.filter(x => x.squirrel);
  //for(let i=0 ;i<JOURNAL[0].events.length;i++){ str =str + JOURNAL[0].events[i]+" "}
